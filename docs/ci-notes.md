...
4. TurboACC：**clone `luci-app-turboacc` + `nft-fullcone`**，并用 `purge-turboacc-duplicates.sh` 删除 feeds/luci 与 `package/feeds` 里的重复副本（重复会导致 `luci-app-turboacc depends on luci-app-turboacc` / `kmod-nft-fullcone selected by kmod-nft-fullcone`）。base `make defconfig` 前用 `ci-turboacc-stash.sh hide` 暂存 clone，defconfig 后再 `unhide` + `ci-enable-turboacc.sh`。
